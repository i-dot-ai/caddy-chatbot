...
def process_chat_message(event: ProcessChatMessageEvent):
    ...
    print(f"{bcolors.OKGREEN} ### FORMATTED MESSAGE QUERY \n\n {message_query} \n\n ### END OF MESSAGE QUERY {bcolors.ENDC}")
    ...
    ai_response, ai_response_timestamp = run_chain(chain, message_query.message, chat_history)

    print(f"{bcolors.OKCYAN}### AI RESPONSE RECEIVED \n\n {ai_response} \n\n ### END OF AI RESPONSE{bcolors.ENDC}")
    print(f"{bcolors.OKBLUE}### START OF AI ANSWER \n\n {ai_response['result']} \n\n ### END OF AI ANSWER{bcolors.ENDC}")
    ...
    store_user_thanked_timestamp(ai_answer, timestamp=datetime.now(), table=responses_table)

    print(f"{bcolors.OKGREEN}### START OF SUPERVISION EVENT \n\n {supervision_event} \n\n ### END OF SUPERVISION EVENT {bcolors.ENDC}")
