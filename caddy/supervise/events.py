...
def receive_new_ai_response(event: SupervisionEvent):
    ...
    match supervisor_space:
        case "Unknown":
            ...
            response_type="text",
            message={"text": "For this query my response requires your supervisors approval before I can send it to you but it doesn't look like your supervisor has registered you to use my support yet, *please speak to a supervisor in order to get set up*"}
            )
        case other:
            request_awaiting, request_approved, request_rejected = create_supervision_request_card(
                user_identifier=user,
                initial_query=initial_query
                )

            new_request_thread, new_request_message_id = send_message_to_supervisor_space(
                space_id=supervisor_space,
                message=request_awaiting
            )
...
def received_rejection(event):
    ...
    message_id = event["message"]["name"].split('/')[3]
    user_message_id = event['common']['parameters']['messageId']
    supervisor_message = event['common']['formInputs']['supervisorResponse']['stringInputs']['value'][0]
    thread_id = event['common']['parameters']['threadId']
    request_message_id = event['common']['parameters']['newRequestId']
...
def introduce_caddy_supervisor(event):
    match event['space']['type']:
        case 'DM':
            return json.dumps({"text": "Hi, I'm the supervisor assistant for Caddy! Caddy is an AI support for Citizens Advice advisers. \n *To get started you will need to register the advisers into your supervision space so their messages come to you, you can do this by typing `/addUser` into the chat, other user management functionality can be seen using /help*"})
        case 'ROOM':
            return json.dumps({"text": f"Hi, thank you for adding me to {event['space']['displayName']}, I'm the supervisor assistant for Caddy! Caddy is an AI support for Citizens Advice advisers. \n\nCaddy uses information from the below sites to form answers: \nGOV UK \nCitizens Advice \nAdviserNet \n\n*To get started you will need to register the advisers into your supervision space so their messages come to you, you can do this by typing `/addUser` into the chat, other user management functionality can be seen using `/help`*"})
